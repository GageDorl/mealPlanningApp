...
import type { MacroPlannerSubmitParams } from './MacroPlannerForm';

interface Props {
  params: MacroPlannerSubmitParams;
  onComplete: () => void;
  onBack: () => void;
}
...
export function MacroRecommendationInline({ params, onComplete, onBack }: Props) {
  ...
  return (
    ...
    >
        ...
        <>
          ...
          <View style={[styles.recCard, { backgroundColor: theme.backgroundElement }]}>
            ...
            {!appliedRec && (
              ...
              >
                <Text style={[styles.applyRecBtnLabel, { color: Colors.accent }]}>Use these values</Text>
              </Pressable>
            )}
          </View>
        </>
        ...
        <>
          ...
          <View style={[styles.guidanceCard, { backgroundColor: theme.backgroundElement }]}>
            {recommendation.guidance.map((line, i) => (
              <Text key={i} style={[styles.guidanceLine, { color: theme.textSecondary }]}>{`• ${line}`}</Text>
            ))}
          </View>
        </>
      )}
      ...
      <View style={[styles.buttonGroup, { marginTop: 24, marginBottom: 32 }]}>
        ...
        <Button label="← Back to form" variant="secondary" onPress={onBack} />
      </View>
    </ScrollView>
  );
}
...
const styles = StyleSheet.create({
  ...
  recMacroItem: { flex: 1, alignItems: 'center', gap: 2 } as ViewStyle,
  recMacroDivider: { width: 1, height: 32 } as ViewStyle,
  ...
  editHint: { fontSize: 12, lineHeight: 16, marginTop: -4, marginBottom: 6 } as TextStyle,
  ...
  inlineInput: { fontSize: 15, minWidth: 60, maxWidth: 90, paddingVertical: 2, paddingHorizontal: 2 } as TextStyle,
  unit: { fontSize: 13, fontWeight: '500', minWidth: 28 } as TextStyle,
  guidanceCard: { borderRadius: BorderRadius.md, padding: 14, gap: 8 } as ViewStyle,
  guidanceLine: { fontSize: 13, lineHeight: 18 } as TextStyle,
